...
# --- Saljscript / saljcoach (valbar komponent; egen fil) ---
# Samma monster som Telefonassistent ovan. Saknas filen forsvinner tray-posten
# och dashboard-raden, och auto-avbockningen i moteslopen hoppas over eftersom
# $script:scriptForm da aldrig skapas. Laddas fore tray-menyn.
$script:ssTillganglig = $false
try {
    . (Join-Path $PSScriptRoot 'SaljScript.ps1')
    $script:ssTillganglig = $true
} catch {
    # Komponenten inte installerad - resten av Diktatorn gar igang anda.
}
...
# ============================================================================
# Inspelningsindikator: liten bricka ovanpa alla fonster medan mikrofonen gar.
#
# Tray-pricken ovan finns kvar, men Windows 11 goemmer tray-ikoner bakom pilen
# som standard - statusen syns alltsa inte utan att man letar efter den. Att
# missa att inspelningen gar kostar at bada hall: antingen tappas ett helt mote,
# eller sa spelas nagot in som inte skulle ha spelats in.
#
# KRITISKT: brickan far inte ta fokus. Dikterad text skrivs in i det fonster som
# HAR fokus (SendInput i WfNative). Ett vanligt TopMost-fonster aktiveras nar det
# visas, och dikteringen hade da hamnat i brickan i stallet for i Word. Darav
# ShowWithoutActivation + WS_EX_NOACTIVATE. WS_EX_TOOLWINDOW haller den ur Alt-Tab.
#
# Blinket ritas i Paint - INTE genom att byta tray-ikon. New-DotIcon lacker ett
# GDI-handtag per anrop (Icon::FromHandle utan DestroyIcon), sa en blinkning i
# sekunden hade atit upp processens 10 000 handtag pa nagra timmar.
# ============================================================================
Add-Type -ReferencedAssemblies System.Windows.Forms -TypeDefinition @"
using System;
using System.Windows.Forms;
public class RecOverlayForm : Form {
    public RecOverlayForm() {
        // Utan dubbelbuffring flimrar brickan vid varje blink.
...
$overlayCfg = Join-Path $root 'diktatorn-overlay.txt'   # sparad position, "x,y"
$script:recForm     = $null
$script:recTimer    = $null
$script:recMode     = $null
$script:recStart    = $null
$script:recText     = ''
$script:recBlink    = $true
$script:recDotColor = [System.Drawing.Color]::FromArgb(210, 60, 60)
$script:recFont     = New-Object System.Drawing.Font('Segoe UI', 9.5, [System.Drawing.FontStyle]::Bold)
$script:recDrag     = $null

# Rundade horn: bade formens Region och bakgrunden ritas fran samma path.
...
function Get-OverlayPos {
    $wa = [System.Windows.Forms.Screen]::PrimaryScreen.WorkingArea
    $def = New-Object System.Drawing.Point(($wa.Right - 200), ($wa.Bottom - 64))
    try {
        if (Test-Path $overlayCfg) {
            $xy = ((Get-Content $overlayCfg -Raw).Trim() -split ',')
            if ($xy.Count -eq 2) {
                $pt = New-Object System.Drawing.Point([int]$xy[0], [int]$xy[1])
                # En sparad position kan hamna utanfor skarmen om skarmuppsattningen
                # andrats. Brickan vore da osynlig OCH omojlig att dra tillbaka, sa
                # falla tillbaka pa hornet i stallet.
                foreach ($s in [System.Windows.Forms.Screen]::AllScreens) {
                    if ($s.WorkingArea.Contains($pt)) { return $pt }
                }
            }
        }
    } catch {}
...
function Save-OverlayPos {
    if (-not $script:recForm -or $script:recForm.IsDisposed) { return }
    try { [System.IO.File]::WriteAllText($overlayCfg, "$($script:recForm.Left),$($script:recForm.Top)") } catch {}
}

function Update-RecOverlayText {
    $el = if ($script:recStart) { (Get-Date) - $script:recStart } else { [TimeSpan]::Zero }
    $label = switch ($script:recMode) {
        'mote'    { SvText 'M~OTE' }
        'journal' { 'JOURNAL' }
        default   { 'DIKTERAR' }
    }
    $script:recText = '{0}   {1}:{2:00}' -f $label, [int]$el.TotalMinutes, $el.Seconds
}

function Show-RecOverlay([string]$mode) {
    $script:recMode  = $mode
    $script:recStart = Get-Date
    $script:recBlink = $true
    $script:recDotColor = if ($mode -eq 'mote') { [System.Drawing.Color]::FromArgb(70, 120, 210) }
                          else { [System.Drawing.Color]::FromArgb(210, 60, 60) }
    Update-RecOverlayText
    if (-not $script:recForm -or $script:recForm.IsDisposed) {
        $f = New-Object RecOverlayForm
        $f.FormBorderStyle = 'None'
        $f.ShowInTaskbar = $false
        $f.TopMost = $true
        $f.StartPosition = 'Manual'
        $f.ClientSize = New-Object System.Drawing.Size(186, 38)
        $f.BackColor = [System.Drawing.Color]::FromArgb(24, 24, 27)
        $rp = New-RoundedPath $f.ClientSize.Width $f.ClientSize.Height 9
        $f.Region = New-Object System.Drawing.Region($rp)
        $rp.Dispose()
        $f.Location = Get-OverlayPos
        ...
        $f.add_Paint({
            ...
            $p = New-RoundedPath $w $h 9
            $bg = New-Object System.Drawing.SolidBrush ([System.Drawing.Color]::FromArgb(24, 24, 27))
            $g.FillPath($bg, $p); $bg.Dispose()
            $pen = New-Object System.Drawing.Pen ([System.Drawing.Color]::FromArgb(64, 64, 72))
            $g.DrawPath($pen, $p); $pen.Dispose()
            $p.Dispose()
            if ($script:recBlink) {
                $db = New-Object System.Drawing.SolidBrush $script:recDotColor
                $g.FillEllipse($db, 14, [int]($h / 2 - 5), 10, 10); $db.Dispose()
            }
            $tb = New-Object System.Drawing.SolidBrush ([System.Drawing.Color]::FromArgb(238, 238, 243))
            $g.DrawString($script:recText, $script:recFont, $tb, [single]34, [single]($h / 2 - 9))
            $tb.Dispose()
        })

        $f.add_MouseDown({ param($s, $e) if ($e.Button -eq 'Left') { $script:recDrag = New-Object System.Drawing.Point($e.X, $e.Y) } })
        $f.add_MouseMove({
            param($s, $e)
            if ($script:recDrag) {
                $s.Left = $s.Left + $e.X - $script:recDrag.X
                $s.Top  = $s.Top  + $e.Y - $script:recDrag.Y
            }
        })
        $f.add_MouseUp({ if ($script:recDrag) { $script:recDrag = $null; Save-OverlayPos } })
        # Bara motesinspelningen gar att stoppa harifran, och bara med dubbelklick.
        # Diktering och journal varar sekunder - dar vore en stoppknapp mest i vagen,
        # medan en feltraff mitt i ett mote kostar hela inspelningen.
        $f.add_MouseDoubleClick({ if ($script:recMode -eq 'mote' -and $script:meeting) { Stop-Meeting } })

        $tip = New-Object System.Windows.Forms.ToolTip
        $tip.SetToolTip($f, (SvText 'Dra f~or att flytta. Dubbelklick stoppar m~otesinspelningen.'))
        $script:recForm = $f
    }
    if (-not $script:recTimer) {
        $script:recTimer = New-Object System.Windows.Forms.Timer
        $script:recTimer.Interval = 600
        $script:recTimer.add_Tick({
            if (-not $script:recForm -or $script:recForm.IsDisposed) { return }
            $script:recBlink = -not $script:recBlink
            Update-RecOverlayText
            $script:recForm.Invalidate()
        })
    }
    $script:recTimer.Start()
    if (-not $script:recForm.Visible) { $script:recForm.Show() }
}

function Hide-RecOverlay {
    if ($script:recTimer) { $script:recTimer.Stop() }
    $script:recMode = $null
    if ($script:recForm -and -not $script:recForm.IsDisposed) { $script:recForm.Hide() }
}
...
$miQuit.add_Click({
    ...
    try { if ($script:recTimer) { $script:recTimer.Stop() } } catch {}
    try { if ($script:recForm -and -not $script:recForm.IsDisposed) { $script:recForm.Dispose() } } catch {}
